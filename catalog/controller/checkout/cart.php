    ...
    if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
      ...
      foreach ($results as $result) {
        if ($this->config->get($result['code'] . '_status')) {
          ...
          $this->data['shipping_methods'][$result['code']] = $this->{'model_shipping_' . $result['code']}->getInfo();
        }
      }
      
      $time_to_order = strtotime(date('d-m-Y') . ' ' . $this->config->get('config_time_to_order'));
      $this->data['shipping_method_msg'] = '';
      $this->load->model('checkout/order');

      $no_of_orders = $this->model_checkout_order->todayDeliverableOrders();

      $order_amount = $total_data[count($total_data) - 1]['value'];
      if ($order_amount >= $this->config->get('config_order_amount')) {

        if (time() < $time_to_order) {
          unset($this->data['shipping_methods']['flat']);
          /*$this->data['shipping_method_msg'] .= "Place the order before " . $this->config->get('config_time_to_order') .
                  " for same day .Hurry Up.";*/
          if ($no_of_orders < $this->config->get('config_max_no_of_orders')) {
            
            $this->data['shipping_method_msg'] .= "Place the order before " . $this->config->get('config_time_to_order') ." for same day delivery.Hurry Up.";
          }
          else {
            $this->data['shipping_method_msg'] .= "Delivery will be in next one or two working days.";
          }
        }
        else {
          $this->data['shipping_method_msg'] .= "Delivery will be in next one or two working days.";
          unset($this->data['shipping_methods']['flat']);
          /*if ($no_of_orders < $this->config->get('config_max_no_of_orders')) {
            $this->data['shipping_method_msg'] .= "Shipping Charges will be applicable for same day delivery.</br>"
                    . "Choose 'Shipping Charge' and apply for same day delivery otherwise choose 'Free Shipping'.";
          }
          else {
            unset($this->data['shipping_methods']['flat']);
          }*/
        ...
        $this->data['shipping_method_msg'] .= "Minimum order amount is " . $this->currency->format($this->config->get('config_order_amount')) . " and place the order before ".$this->config->get('config_time_to_order')." for free shipping and on the same day delivery.";
        if (time() > $time_to_order) {
          $this->data['shipping_method_msg'] .= "</br>Time is over.On the same day delivery is not available.";
        }else {
          $this->data['shipping_method_msg'] .= "</br>Please choose 'Shippping Charge' and apply for same day delivery other wise choose 'Free Shipping' then delivery will be in next one or two working days.";
        }
        //unset($this->data['shipping_methods']['free']);
      }
      //echo $this->data['shipping_method_msg'];
      if(!empty($shipping_method)) {
        $pieces = explode('.', $shipping_method);
        $this->data['shipping_method'] = $shipping_method;
        $this->session->data['shipping_method'] = $this->data['shipping_methods'][$pieces[0]]['quote'][$pieces[1]];
      }
      if (count($this->data['shipping_methods']) == 1) {
        foreach ($this->data['shipping_methods'] as $code => $method) {
          $this->session->data['shipping_method'] = $method['quote'][$code];
        }
      }
