...
class ModelToolExportImport extends Model {
    ...
    if(!empty($category_id)) {
      ...
      $sql = "INSERT INTO `".DB_PREFIX."category` (`image`, `parent_id`, `top`, `column`, `sort_order`, `date_added`, `date_modified`, `status`) VALUES ";
      $sql .= "('$image_name', $parent_id, $top, $columns, $sort_order, ";
    }		
	...
	public function upload( $filename, $incremental=false ) {
		...
		try {
			...
			return true;
		} catch (Exception $e) {echo 'lkc';exit;
			$errstr = $e->getMessage();
			$errline = $e->getLine();
			$errfile = $e->getFile();
			$errno = $e->getCode();
